...
public interface UserMapper {
  ...
  @Insert(
      "insert into t_user(username,password,name,email,create_time,status) values(#{username},#{password}"
          + ",#{name},#{email},now(),0)")
  ...
  @Select(
      "<script>"
          + "select date_format(create_date,'%Y-%m-%d') as date,count(uiid) as num from t_user_info "
  ...
  @Select(
      "<script>"
          + "select date_format(a.create_date,'%Y-%m-%d') as date,count(distinct a.uiid) as num from t_pay_order a "
          + "inner join (select uiid from t_user_info where create_date&gt;=date_format("
  ...
  @Select(
      "<script>"
          + "select date_format(create_date,'%Y-%m-%d') as date,sum(amount) as num from t_pay_order "
          + "where date_format(create_date,'%Y-%m-%d')&gt;=date_format(date_add(now(),"
